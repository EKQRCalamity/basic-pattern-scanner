...
use crate::error::{InternalResult, Error};
...
impl Pattern {
	pub fn new(bytes: Vec<u8>, mask: Vec<u8>, mask_type: MaskType) -> InternalResult<Self> {
		if bytes.len() > mask.len() {
			Err(Error::InvalidPattern { kind: PatternKind::Ida, repr: PatternRepr::BytesWithMask, hint: Some(Cow::Owned("Mask bytes must be atleast the same amount as the bytes of the pattern.".to_owned())) })
		} else {
			let masked_bytes = bytes.iter().zip(mask.iter()).map(|(b, m)| b & m).collect();
			Ok(Self { bytes, mask, mask_type, masked_bytes })
		}
	}
	...
	pub fn from_ida_str(pattern: &str) -> InternalResult<Self> {
		...
		for p in pattern.split_whitespace() {
			if p == "?" || p == "??" {
				...
				let b = u8::from_str_radix(p, 16).map_err(|o| Error::InvalidPattern { kind: PatternKind::Ida, repr: PatternRepr::BytesWithMask, hint: Some(Cow::Owned(o.to_string()))})?;
				bytes.push(b);
				mask.push(0xFF);
			}
		}
